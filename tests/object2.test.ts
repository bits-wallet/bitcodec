...
import datas from "./data/object2";
...
datas.forEach((data, i) => {
  test("object encodingLength i:" + i, () => {
    expect(data.codec.encodingLength(hex2buffer(data.obj) as never)).toEqual(data.encodingLength);
  });
});
...
datas.forEach((data, i) => {
  test("object encode i:" + i, () => {
    const hex = buffer2hex(data.codec.encode(hex2buffer(data.obj) as never));

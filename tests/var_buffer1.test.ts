...
import datas from "./data/var_buffer1";

datas.forEach((data, i) => {
  test("var_buffer1 encodeBytes i:" + i, () => {
    expect(data.codec.encodeBytes).toEqual(0);
  });
});

datas.forEach((data, i) => {
  test("var_buffer1 decodeBytes i:" + i, () => {
    expect(data.codec.decodeBytes).toEqual(0);
  });
});

datas.forEach((data, i) => {
  test("var_buffer1 encodingLength i:" + i, () => {
    expect(data.codec.encodingLength(hex2buffer(data.obj))).toEqual(data.encodingLength);
  });
});

datas.forEach((data, i) => {
  test("var_buffer1 decode i:" + i, () => {
